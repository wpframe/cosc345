...
def extract_tickers(input_file, output_file):
    with open(input_file, 'r') as csv_file:
        ...
        data = [{'Symbol': row['Symbol'], 'Market Cap': float(row['Market Cap'].replace(
            ',', '')) if row['Market Cap'] else 0} for row in reader]
...
if __name__ == "__main__":
    ...
    print(
        f"Top 1000 stock tickers by market cap have been saved to {output_file}.")
